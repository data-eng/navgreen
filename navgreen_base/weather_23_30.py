...
def detailed(html):
    ...
    for perhour in perhours:
        ...
        # Format the next day's date as dd/mm
        next_day_str = next_day.strftime('%d/%m/%Y')
        # Check if the current html date-string matches the next day's date
        is_next_day = curr_forecast_date == next_day_str

        # The task is to gather data the night of the current date, for the next date
        if is_next_day:
            data = {}

            data['ACCESS_DATETIME'] = today.strftime('%d/%m/%Y %H:%M:%S')
...
def call_function_daily(func, *args, **kwargs):
    ...
    # Schedule the job to run every day at a specific hour
    schedule.every().day.at('23:30').do(job)

    while True:
        try:
            schedule.run_pending()
            time.sleep(20)  # Check for pending tasks 20 seconds
        except Exception as e:
            logger.error(f"An error occurred in the scheduling loop: {e}")
            time.sleep(5)  # Short delay before retrying the scheduling loop


if __name__ == '__main__':
    url = 'https://meteo.gr/cf.cfm?city_id=88'

    # Call the function daily at 23:00
    call_function_daily(get_predictions, url)
