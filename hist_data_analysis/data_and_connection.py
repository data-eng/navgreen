...
def write_data(row, url, token, organization):
    ...
    api.write(bucket="stasinos-playground", org=organization, record=p)
...
df = pd.DataFrame(columns=['Date_time_local', 'DATETIME', 'PVT_IN_TO_DHW', 'PVT_OUT_FROM_DHW', 'PVT_IN_TO_SOLAR_BUFFER',
...
event, values = sg.Window('Connect to PLC', [[sg.Text('Connect to PLC->'),
                                              sg.Listbox(['True', 'False'], size=(100, 10), key='LB')],
            ...
            if client.connect():
                ...
                while True:
                    ...
                    if result.isError():
                        ...
                        df.to_csv('C:/Users/res4b/Desktop/modbus_tcp_ip/data_from_plc.csv',
                                  mode='w', index=False)
                        ##############################################################################
