...
def train(data, classes, epochs, patience, lr, criterion, model, optimizer, scheduler, seed, y, visualize=False):
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            mfn = utils.get_path(dirs=["models", ylabel, "transformer", "trained_new_best_time_repr", str(seed)], name="transformer.pth")
            ...
            if visualize:
                utils.visualize(type="heatmap",
                                ...
                                path=utils.get_path(dirs=["models", ylabel, "transformer", "trained_new_best_time_repr", str(seed)]))
    ...
    cfn = utils.get_path(dirs=["models", ylabel, "transformer", "trained_new_best_time_repr", str(seed)], name="train_checkpoints.json")
    checkpoints.update({'epochs': epoch+1})
    utils.save_json(data=checkpoints, filename=cfn)
    
    if visualize:

        cfn = utils.get_path(dirs=["models", ylabel, "transformer", "trained_new_best_time_repr", str(seed)], name="train_losses.json")
